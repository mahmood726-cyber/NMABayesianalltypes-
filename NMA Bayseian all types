...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    if (pt == "Traceplot") {
      ...
      tn <- grep("^theta\\[", colnames(mat), value=TRUE)
      dd <- rv$sample_data
      if (length(tn) > 0 && "Effect" %in% names(dd)) {
        fitted <- colMeans(mat[, tn, drop=FALSE])
        n <- min(length(fitted), length(dd$Effect))
        observed <- dd$Effect[seq_len(n)]
        fitted <- fitted[seq_len(n)]
        lim <- range(c(observed, fitted), na.rm=TRUE)
        plot(observed, fitted, pch=19,
             xlab="Observed effect",
             ylab="Posterior mean fitted effect",
             main="Posterior Predictive Check",
             xlim=lim, ylim=lim)
        abline(0, 1, col="red", lty=2)
      } else {
        plot.new(); text(0.5,0.5,"Posterior fitted effects unavailable for current model")
      }
      storeBasePlot()
